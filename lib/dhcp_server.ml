...
(* | SUBNET_MASK -> r find_subnet_mask (fun x -> Subnet_mask x) *)
      | TIME_OFFSET -> r find_time_offset (fun x -> Time_offset x)
      ...
      | HOSTNAME -> r find_hostname (fun x -> Hostname x)
      | BOOTFILE_SIZE -> r find_bootfile_size (fun x -> Bootfile_size x)
      | MERIT_DUMPFILE -> r find_merit_dumpfile (fun x -> Merit_dumpfile x)
      | DOMAIN_NAME -> r find_domain_name (fun x -> Domain_name x)
      | SWAP_SERVER -> r find_swap_server (fun x -> Swap_server x)
      | ROOT_PATH -> r find_root_path (fun x -> Root_path x)
      | EXTENSION_PATH -> r find_extension_path (fun x -> Extension_path x)
      | IPFORWARDING -> r find_ipforwarding (fun x -> Ipforwarding x)
      | NLSR -> r find_nlsr (fun x -> Nlsr x)
      | POLICY_FILTERS -> m collect_policy_filters (fun x -> Policy_filters x)
      | MAX_DATAGRAM -> r find_max_datagram (fun x -> Max_datagram x)
      | DEFAULT_IP_TTL -> r find_default_ip_ttl (fun x -> Default_ip_ttl x)
      | PMTU_AGEING_TIMO -> r find_pmtu_ageing_timo (fun x -> Pmtu_ageing_timo x)
      | PMTU_PLATEAU_TABLE ->
        r find_pmtu_plateau_table (fun x -> Pmtu_plateau_table x)
      | INTERFACE_MTU -> r find_interface_mtu (fun x -> Interface_mtu x)
      | ALL_SUBNETS_LOCAL -> r find_all_subnets_local (fun x -> All_subnets_local x)
      | BROADCAST_ADDR -> r find_broadcast_addr (fun x -> Broadcast_addr x)
      | PERFORM_MASK_DISCOVERY ->
        r find_perform_mask_discovery (fun x -> Perform_router_disc x)
      | MASK_SUPPLIER -> r find_mask_supplier (fun x -> Mask_supplier x)
      | PERFORM_ROUTER_DISC ->
        r find_perform_router_disc (fun x -> Perform_router_disc x)
      | ROUTER_SOL_ADDR -> r find_router_sol_addr (fun x -> Router_sol_addr x)
      | STATIC_ROUTES -> m collect_static_routes (fun x -> Static_routes x)
      | TRAILER_ENCAPSULATION ->
        r find_trailer_encapsulation (fun x -> Trailer_encapsulation x)
      | ARP_CACHE_TIMO -> r find_arp_cache_timo (fun x -> Arp_cache_timo x)
      | ETHERNET_ENCAPSULATION ->
        r find_ethernet_encapsulation (fun x -> Ethernet_encapsulation x)
      | TCP_DEFAULT_TTL -> r find_tcp_default_ttl (fun x -> Tcp_default_ttl x)
      | TCP_KEEPALIVE_INTERVAL ->
        r find_tcp_keepalive_interval (fun x -> Tcp_keepalive_interval x)
      | TCP_KEEPALIVE_GARBAGE ->
        r find_tcp_keepalive_garbage (fun x -> Tcp_keepalive_garbage x)
      | NIS_DOMAIN -> r find_nis_domain (fun x -> Nis_domain x)
      | NIS_SERVERS -> m collect_nis_servers (fun x -> Nis_servers x)
      | NTP_SERVERS -> m collect_ntp_servers (fun x -> Ntp_servers x)
      | VENDOR_SPECIFIC -> r find_vendor_specific (fun x -> Vendor_specific x)
      ...
      | NETBIOS_NODE -> r find_netbios_node (fun x -> Netbios_node x)
      | NETBIOS_SCOPE -> r find_netbios_scope (fun x -> Netbios_scope x)
      ...
      | REQUEST_IP -> r find_request_ip (fun x -> Request_ip x)
      | IP_LEASE_TIME -> None   (* Previously included *)
      | OPTION_OVERLOAD -> r find_option_overload (fun x -> Option_overload x)
      | MESSAGE_TYPE -> r find_message_type (fun x -> Message_type x)
      | SERVER_IDENTIFIER ->
        r find_server_identifier (fun x -> Server_identifier x)
      | PARAMETER_REQUESTS -> None (* Senseless *)
      | MESSAGE -> r find_message (fun x -> Message x)
      | MAX_MESSAGE -> r find_max_message (fun x -> Max_message x)
      | RENEWAL_T1 -> None (* Previously included *)
      | REBINDING_T2 -> None (* Previously included *)
      | VENDOR_CLASS_ID -> r find_vendor_class_id (fun x -> Vendor_class_id x)
      | CLIENT_ID -> None (* Senseless *)
      | NETWARE_IP_DOMAIN ->
        r find_netware_ip_domain (fun x -> Netware_ip_domain x)
      | NETWARE_IP_OPTION ->
        r find_netware_ip_option (fun x -> Netware_ip_option x)
      | NIS_PLUS_DOMAIN -> r find_nis_plus_domain (fun x -> Nis_plus_domain x)
      | NIS_PLUS_SERVERS ->
        m collect_nis_plus_servers (fun x -> Nis_plus_servers x)
      | TFTP_SERVER_NAME -> r find_tftp_server_name (fun x -> Tftp_server_name x)
      | BOOTFILE_NAME -> r find_bootfile_name (fun x -> Bootfile_name x)
      ...
      | USER_CLASS -> r find_user_class (fun x -> User_class x)
      | DIRECTORY_AGENT -> r find_directory_agent (fun x -> Directory_agent x)
      | SERVICE_SCOPE -> r find_service_scope (fun x -> Service_scope x)
      | RAPID_COMMIT -> r find_rapid_commit (fun _ -> Rapid_commit)
      | CLIENT_FQDN -> r find_client_fqdn (fun x -> Client_fqdn x)
      | RELAY_AGENT_INFORMATION ->
        r find_relay_agent_information (fun x -> Relay_agent_information x)
      | ISNS -> r find_isns (fun x -> Isns x)
      | NDS_SERVERS -> r find_nds_servers (fun x -> Nds_servers x)
      | NDS_TREE_NAME -> r find_nds_tree_name (fun x -> Nds_tree_name x)
      | NDS_CONTEXT -> r find_nds_context (fun x -> Nds_context x)
      | BCMCS_CONTROLLER_DOMAIN_NAME_LIST ->
        r find_bcmcs_controller_domain_name
      ...
      | AUTHENTICATION -> r find_authentication (fun x -> Authentication x)
      | CLIENT_LAST_TRANSACTION_TIME ->
        r find_client_last_transaction_time
          (fun x -> Client_last_transaction_time x)
      | ASSOCIATED_IPS -> m collect_associated_ips (fun x -> Associated_ips x)
      | CLIENT_SYSTEM -> r find_client_system (fun x -> Client_system x)
      | CLIENT_NDI -> r find_client_ndi (fun x -> Client_ndi x)
      | LDAP -> r find_ldap (fun x -> Ldap x)
      | UUID_GUID -> r find_uuid_guid (fun x -> Uuid_guid x)
      | USER_AUTH -> r find_user_auth (fun x -> User_auth x)
      | GEOCONF_CIVIC -> r find_geoconf_civic (fun x -> Geoconf_civic x)
      | PCODE -> r find_pcode (fun x -> Pcode x)
      | TCODE -> r find_tcode (fun x -> Tcode x)
      | NETINFO_ADDRESS -> r find_netinfo_address (fun x -> Netinfo_address x)
      | NETINFO_TAG -> r find_netinfo_tag (fun x -> Netinfo_tag x)
      | URL -> r find_url (fun x -> Url x)
      | AUTO_CONFIG -> r find_auto_config (fun x -> Auto_config x)
      | NAME_SERVICE_SEARCH ->
        r find_name_service_search (fun x -> Name_service_search x)
      | SUBNET_SELECTION -> r find_subnet_selection (fun x -> Subnet_selection x)
      | DOMAIN_SEARCH -> r find_domain_search (fun x -> Domain_search x)
      | SIP_SERVERS -> r find_sip_servers (fun x -> Sip_servers x)
      | CLASSLESS_STATIC_ROUTE ->
        r find_classless_static_route (fun x -> Classless_static_route x)
      | CCC -> r find_ccc (fun x -> Ccc x)
      | GEOCONF -> r find_geoconf (fun x -> Geoconf x)
      | VI_VENDOR_CLASS -> r find_vi_vendor_class (fun x -> Vi_vendor_class x)
      | VI_VENDOR_INFO -> r find_vi_vendor_info (fun x -> Vi_vendor_info x)
      | PXE_128 -> r find_pxe_128 (fun x -> Pxe_128 x)
      | PXE_129 -> r find_pxe_129 (fun x -> Pxe_129 x)
      | PXE_130 -> r find_pxe_130 (fun x -> Pxe_130 x)
      | PXE_131 -> r find_pxe_131 (fun x -> Pxe_131 x)
      | PXE_132 -> r find_pxe_132 (fun x -> Pxe_132 x)
      | PXE_133 -> r find_pxe_133 (fun x -> Pxe_133 x)
      | PXE_134 -> r find_pxe_134 (fun x -> Pxe_134 x)
      | PXE_135 -> r find_pxe_135 (fun x -> Pxe_135 x)
      | PANA_AGENT -> r find_pana_agent (fun x -> Pana_agent x)
      | V4_LOST -> r find_v4_lost (fun x -> V4_lost x)
      | CAPWAP_AC_V4 -> r find_capwap_ac_v4 (fun x -> Capwap_ac_v4 x)
      | IPV4_ADDRESS_MOS -> r find_ipv4_address_mos (fun x -> Ipv4_address_mos x)
      | IPV4_FQDN_MOS -> r find_ipv4_fqdn_mos (fun x -> Ipv4_fqdn_mos x)
      | SIP_UA_DOMAINS -> r find_sip_ua_domains (fun x -> Sip_ua_domains x)
      | IPV4_ADDRESS_ANDSF ->
        r find_ipv4_address_andsf (fun x -> Ipv4_address_andsf x)
      | GEOLOCK -> r find_geolock (fun x -> Geolock x)
      | FORCENEW_NONCE_CAPABLE ->
        r find_forcenew_nonce_capable (fun x -> Forcenew_nonce_capable x)
      | RDNSS_SELECTION -> r find_rdnss_selection (fun x -> Rdnss_selection x)
      | MISC_150 -> r find_misc_150 (fun x -> Misc_150 x)
      | STATUS_CODE -> r find_status_code (fun x -> Status_code x)
      | ABSOLUTE_TIME -> r find_absolute_time (fun x -> Absolute_time x)
      | START_TIME_OF_STATE ->
        r find_start_time_of_state (fun x -> Start_time_of_state x)
      | QUERY_START_TIME -> r find_query_end_time (fun x -> Query_start_time x)
      | QUERY_END_TIME -> r find_query_end_time (fun x -> Query_end_time x)
      | DHCP_STATE -> r find_dhcp_state (fun x -> Dhcp_state x)
      | DATA_SOURCE -> r find_data_source (fun x -> Data_source x)
      | V4_PCP_SERVER -> r find_v4_pcp_server (fun x -> V4_pcp_server x)
      | V4_PORTPARAMS -> r find_v4_portparams (fun x -> V4_portparams x)
      | DHCP_CAPTIVE_PORTAL ->
        r find_dhcp_captive_portal (fun x -> Dhcp_captive_portal x)
      | ETHERBOOT_175 -> r find_etherboot_175 (fun x -> Etherboot_175 x)
      | IP_TELEFONE -> r find_ip_telefone (fun x -> Ip_telefone x)
      | ETHERBOOT_177 -> r find_etherboot_177 (fun x -> Etherboot_177 x)
      | PXE_LINUX -> r find_pxe_linux (fun x -> Pxe_linux x)
      | CONFIGURATION_FILE ->
        r find_configuration_file (fun x -> Configuration_file x)
      | PATH_PREFIX -> r find_path_prefix (fun x -> Path_prefix x)
      | REBOOT_TIME -> r find_reboot_time (fun x -> Reboot_time x)
      | OPTION_6RD -> r find_option_6rd (fun x -> Option_6rd x)
      | V4_ACCESS_DOMAIN -> r find_v4_access_domain (fun x -> V4_access_domain x)
      | SUBNET_ALLOCATION ->
        r find_subnet_allocation (fun x -> Subnet_allocation x)
      | VIRTUAL_SUBNET_SELECTION ->
        r find_virtual_subnet_selection (fun x -> Virtual_subnet_selection x)
      | WEB_PROXY_AUTO_DISC ->
        r find_web_proxy_auto_disc (fun x -> Web_proxy_auto_disc x)
